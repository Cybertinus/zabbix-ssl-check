...
expire_string="$(openssl s_client -servername "${domainname}" -connect "${domainname}":"${service}" < /dev/null 2> /dev/null | openssl x509 -text | awk -F: '/Not After/ {gsub(/^ /, "", $2); print $2 ":" $3 ":" $4}')"
